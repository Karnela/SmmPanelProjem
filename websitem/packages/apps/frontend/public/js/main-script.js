...
// main-script.js içindeki $(document).ready(...) bloğunun içine...

    // ... bir önceki adımda eklediğimiz login formu kodları burada ...


    // ======================================================
    // ========== YENİ SİPARİŞ FORMU YÖNETİMİ ===============
    // ======================================================
    $("#newOrderForm").on("submit", function(event) {
        ...
        // 2. GİRİŞ KONTROLÜ: Tarayıcı hafızasından Access Token'ı al
        const accessToken = localStorage.getItem("accessToken");
        if (!accessToken) {
            showNotification("Sipariş vermek için giriş yapmanız gerekiyor.", true);
            // Kullanıcıyı 2 saniye sonra giriş sayfasına yönlendir
            setTimeout(() => { window.location.href = "../index.html"; }, 2000);
            return;
        }

        // 3. Formdaki tüm verileri bir JavaScript nesnesinde topla
        const orderData = {
            // Not: serviceSelect'in value'sunun servis ID'si olduğunu varsayıyoruz
            service_id: parseInt($("#serviceSelect").val()), 
            kanal_id: parseInt($("#channelSelect").val()), // Kanal seçimi için bir select olduğunu varsayalım
            hedef_url: $("#orderLink").val(),
            hedef_miktar: parseInt($("#orderQuantity").val()),
            
            // İsteğe bağlı alanları, sadece doluysa ekle
            runs: $("#orderRuns").val() ? parseInt($("#orderRuns").val()) : undefined,
            interval: $("#orderInterval").val() ? parseInt($("#orderInterval").val()) : undefined,
            comments: $("#orderComments").val() || undefined,
            hashtag: $("#orderHashtag").val() || undefined,
            usernames: $("#orderUsernames").val() || undefined,
            answer_number: $("#orderAnswerNumber").val() || undefined
            // ...diğer abonelik alanları da benzer şekilde eklenebilir...
        };

        // 4. API'ye AJAX isteği gönder
        $.ajax({
            url: "http://localhost:8000/abonelikler/siparisler", // Sipariş oluşturma endpoint'imiz
            type: "POST",
            data: JSON.stringify(orderData), // Veriyi JSON formatında gönder
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            beforeSend: function(xhr) {
                // 5. EN ÖNEMLİ KISIM: İsteğin başlığına (header) Bearer Token'ı ekle
                // API, bu başlık sayesinde kimin istek yaptığını anlar.
                xhr.setRequestHeader("Authorization", "Bearer " + accessToken);
            },
            success: function(response) {
                // 6. Başarılı olursa: Kullanıcıya bilgi ver
                showNotification("Siparişiniz başarıyla alındı! Sipariş ID: " + response.siparis_id, false);
                // Formu temizle
                $("#newOrderForm")[0].reset(); 
            },
            error: function(jqXHR, textStatus, errorThrown) {
                // 7. Hata olursa: Hata mesajını göster
    ...
    // ... $(document).ready içindeki diğer kodlarınız ...

// Bu fonksiyonu, kullanıcıya geri bildirim (başarı/hata mesajları) göstermek için kullanalım.
function showNotification(message, isError = false) {
    const notifyBox = $('#notifyText');
    const notification = $('#notification');

    // HTML yapınızda bildirim kutusuna özel class'lar ekleyerek renklendirebilirsiniz.
    // Şimdilik sadece mesajı gösteriyoruz.
    notifyBox.text(message);

    // jQuery'nin fadeIn/fadeOut efektleriyle bildirimi gösterip saklayalım.
    notification.fadeIn();
    setTimeout(function() {
...
$(document).ready(function() {
    ...
    // ======================================================
    // =============== GİRİŞ FORMU YÖNETİMİ =================
    // ======================================================
    $("#loginForm").on("submit", function(event) {
        
        // 1. Tarayıcının varsayılan form gönderme (sayfa yenileme) davranışını engelle
        event.preventDefault();

        // 2. Formdaki input değerlerini al
        const username = $("#username").val(); // E-posta adresi bu alana girilecek
        const password = $("#password").val();

        // 3. Basit bir kontrol
        if (!username || !password) {
            showNotification("Kullanıcı adı ve şifre boş bırakılamaz.", true);
            return;
        }

        // 4. API'nin beklediği formatta veriyi hazırla (application/x-www-form-urlencoded)
        const formData = new URLSearchParams();
        formData.append('username', username);
        formData.append('password', password);

        // 5. FastAPI'deki /auth/token endpoint'ine AJAX ile POST isteği gönder
        $.ajax({
            url: "http://localhost:8000/auth/token", // Çalışan API sunucumuzun adresi
            type: "POST",
            data: formData.toString(),
            contentType: "application/x-www-form-urlencoded",
            success: function(response) {
                // 6. Başarılı olursa: Dönen token'ı tarayıcının hafızasına kaydet
                localStorage.setItem("accessToken", response.access_token);
                
                showNotification("Giriş başarılı! Panele yönlendiriliyorsunuz...", false);

                // 7. Kullanıcıyı 1 saniye sonra panelin ana sayfasına yönlendir
                setTimeout(function() {
                    window.location.href = "./folder/neworder.html";
                }, 1000);
            },
            error: function(jqXHR, textStatus, errorThrown) {
                // 8. Hata olursa: Sunucudan gelen hata mesajını göster
